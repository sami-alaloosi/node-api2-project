...
function App() {
  ...
  const submitAdd = (event) =>{
    ...
    axios.post('http://localhost:4000/api/posts', {title: addName, contents:addBio } )
    .then(()=>{
      axiosUsersGet()
      setAddname("")
      setAddBio("")
  ...
  const axiosDelete = (deleteId) =>{
    axios.delete(`http://localhost:4000/api/posts/${deleteId}`)
    .then(()=> axiosUsersGet())
    .catch((err)=> console.log(err))
  }
  ...
  const submitUpdate = (event, updateID) =>{
    event.preventDefault()
    axios.put(`http://localhost:4000/api/posts/${updateID}`, {title: updateName, contents:updateBio } )
    .then(()=>{
      axiosUsersGet()
      setUpdateName("")
      setUpdateBio("")
  ...
  return (
    <div className="App">
      <header className="App-header">
        {posts.map((post) => {
          return (
            <div key={post.id} className="red">
              <div> title: {post.title}</div>
              <div> contents: {post.contents}</div>

              <button onClick={()=>axiosDelete(post.id)}>Delete</button>
              <button onClick={()=> updateID === post.id? setUpdateID("no"): setUpdateID(post.id)}>Update</button>

              <div className={post.id === updateID ? "show" : "no-show"}>
          ...
          <button type="submit">Update Post Information</button>
        </form>
              </div>
